...
export default function SubjectsPage() {
    ...
    const loadData = async () => {
        try {
            const [subjectsRes, classesRes, teachersRes, departmentsRes] = await Promise.all([
                api.get('/subjects'),
    ...
    const openCreateDialog = () => {
        setEditingId(null);
        setFormData({ name: '', code: '', class_id: '', teacher_id: '', final_max_marks: '30', assignment_scope: 'full_class', department_id: '' });
    ...
    const handleEdit = async (subject: Subject) => {
        ...
        setFormData({
            ...
            department_id: subject.department_id ? subject.department_id.toString() : '',
        });
        setSelectedStudentIds([]);
        setStudentSearch('');
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (formData.assignment_scope === 'department' && !formData.department_id) {
        ...
        const payload: any = {
            ...
            final_max_marks: parseInt(formData.final_max_marks),
            assignment_scope: formData.assignment_scope,
            department_id: formData.assignment_scope === 'department' ? parseInt(formData.department_id) : null,
            student_ids: formData.assignment_scope === 'selected_students' ? selectedStudentIds : [],
        };
        try {
            if (editingId) {
        ...
        <AppLayout title="Subjects" showBack>
            <div className="p-4 lg:p-6 max-w-6xl mx-auto space-y-8 pb-24">
                ...
                <Dialog open={showDialog} onOpenChange={setShowDialog}>
                    <DialogContent
                        className="
                            w-full max-w-[95vw] sm:max-w-[720px]
                            p-0 gap-0
                            flex flex-col
                    ...
                    >
                        ...
                        <div className="px-6 py-4 border-b border-slate-100 shrink-0">
                            ...
                            <p className="text-[12px] text-slate-500 mt-0.5">
                                {editingId ? 'Update subject details and assignment.' : 'Define a new subject, assign faculty and students.'}
                            </p>
                        </div>
                            ...
                            <div className="px-6 py-5 space-y-7">
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                        <div className="space-y-1.5">
                                            ...
                                            <Input
                                                ...
                                                required
                                                className="h-9 text-[13px] rounded-lg border-slate-200 bg-slate-50 focus:bg-white transition-colors"
                                            />
                                        </div>
                                        <div className="space-y-1.5">
                                            <Label className="text-[12px] font-semibold text-slate-600">
                                ...
                                <div>
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <div className="space-y-1.5">
                                            ...
                                            >
                                                <SelectTrigger className="h-9 text-[13px] rounded-lg border-slate-200 bg-slate-50">
                                                    <SelectValue placeholder="Select a class…" />
                                                </SelectTrigger>
                                                <SelectContent position="popper" side="bottom" className="max-h-60 overflow-y-auto">
                                                    {classes.map((cls) => (
                                                        <SelectItem key={cls.id} value={cls.id.toString()} className="text-[13px]">
                                                            {cls.name}
                                                        </SelectItem>
                                                    ))}
                                                </SelectContent>
                                        ...
                                        {/* Teacher — native searchable select for performance */}
                                        <div className="space-y-1.5">
                                            <Label className="text-[12px] font-semibold text-slate-600">
                                            ...
                                            <div className="flex flex-col sm:flex-row gap-2">
                                                {(['full_class', 'selected_students', 'department'] as const).map((scope) => (
                                                    <button
                                                        key={scope}
                                                        type="button"
                                                        onClick={() => setFormData({ ...formData, assignment_scope: scope })}
                                                        className={`
                                                            flex-1 py-2 px-3 rounded-lg border text-[12px] font-semibold transition-all
                                                            ${formData.assignment_scope === scope
                                        ...
                                        {formData.assignment_scope === 'department' && (() => {
                                            const selectedDeptName = departments.find(d => d.id.toString() === formData.department_id)?.name || '';
                                            return (
                                                    ...
                                                    {formData.department_id && (
                                                        <div className="space-y-1.5 animate-in fade-in duration-200">
                                                            <Label className="text-[12px] font-semibold text-slate-600 flex items-center gap-1.5">
                                                                ...
                                                                Students in {selectedDeptName}
                                                                {formData.class_id && (
                                                                    <span className="ml-auto text-[11px] font-bold text-purple-700 bg-purple-50 px-2 py-0.5 rounded-full">
                                                                        {classStudents.filter(s => s.department === selectedDeptName).length} matching
                                                                    </span>
                                                                )}
                                                            </Label>
                                                            {!formData.class_id ? (
                                                                ...
                                                                <div className="border border-slate-200 rounded-lg overflow-hidden max-h-44 overflow-y-auto divide-y divide-slate-50 bg-white">
                                                                    {classStudents.filter(s => s.department === selectedDeptName).map(s => (
                                                                    ...
                                                                    {classStudents.filter(s => s.department === selectedDeptName).length === 0 && (
                                                    ...
                                                    <div className="border border-slate-200 rounded-lg overflow-hidden">
                                                        ...
                                                        <div className="flex items-center gap-2 px-3 py-2 border-b border-slate-100 bg-slate-50/80">
                                                            ...
                                                            <input
                                                                ...
                                                                className="flex-1 text-[12px] bg-transparent outline-none placeholder:text-slate-400 text-slate-700"
                                                            />
                                                            <button
                                                                type="button"
                                                                onClick={() => {
                                                                ...
                                                                className="text-[10px] font-black text-emerald-700 uppercase tracking-wide shrink-0 hover:text-emerald-800 transition-colors"
                                                            >
                                                                {(() => {
                                                                    const filtered = classStudents.filter(s =>
                                                                        !studentSearch ||
                                                                    ...
                                                                    return filtered.every(s => selectedStudentIds.includes(s.id)) ? 'Deselect' : 'Select All';
                                                                })()}
                                                            </button>
                                                        </div>
                                                        {/* List */}
                                                        <div className="max-h-44 overflow-y-auto divide-y divide-slate-50">
                                                            {classStudents
                                                                ...
                                                                .map(s => {
                                                                    ...
                                                                    return (
                                                                        <label
                                                                            key={s.id}
                                                                            className={`flex items-center gap-3 px-3 py-2 cursor-pointer select-none transition-colors ${checked ? 'bg-emerald-50/60' : 'hover:bg-slate-50'}`}
                                                                        >
                                                                            <input
                                                                                type="checkbox"
                                                                                className="accent-[#008f6c] w-3.5 h-3.5 shrink-0 rounded"
                                                                                checked={checked}
                                                                                onChange={() =>
                                                            ...
                                                            {classStudents.length === 0 && (
                                                                <p className="text-center text-[12px] text-slate-400 py-5">No students in this class.</p>
                                                            )}
                                                        </div>
                                                    </div>
                                                )}
                                ...
                                <div>
                                    ...
                                    <div className="space-y-1.5 max-w-[160px]">
                                        <Label className="text-[12px] font-semibold text-slate-600">
                                            Final Max Marks <span className="text-rose-500">*</span>
                                        </Label>
                                        <div className="relative">
                                            <Input
                                                type="number"
                                                ...
                                                max="100"
                                                required
                                                className="h-9 text-[13px] rounded-lg border-slate-200 bg-slate-50 focus:bg-white pr-10 transition-colors"
                                            />
                        ...
                        {/* ── Sticky Footer ── */}
                        <div className="shrink-0 px-6 py-3.5 border-t border-slate-100 bg-white flex items-center justify-end gap-2">
                            <Button
                                type="button"
                                variant="ghost"
                                size="sm"
                                onClick={() => setShowDialog(false)}
                                className="h-8 px-4 text-[13px] font-semibold text-slate-600 hover:text-slate-800 hover:bg-slate-100 rounded-lg"
                            >
                                Cancel
                            </Button>
                            <Button
                                ...
                                form="subject-form"
                                size="sm"
                                className="h-8 px-5 text-[13px] font-bold bg-[#008f6c] hover:bg-[#007a5c] text-white rounded-lg shadow-sm"
                            >
