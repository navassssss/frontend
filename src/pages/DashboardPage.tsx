...
export default function DashboardPage() {
  ...
  return (
    <AppLayout title="Dashboard">
      ...
      {/* ── DESKTOP VIEW ── */}
      <div className="hidden lg:flex flex-col h-full p-6 space-y-6">
        ...
        <div>
          ...
          <p className="text-muted-foreground mt-0.5 text-xs">
            {isPrincipal ? 'Reviewing metrics for your institution' : "Your overview for today"}
          </p>
        </div>
        ...
        <div className="grid grid-cols-4 gap-3">
          {loading ? (
            ...
            return (
              <div
                key={stat.label}
                className="bg-card border border-border rounded-xl p-4 shadow-sm hover:shadow-md transition-shadow cursor-default"
              >
                <p className="text-[10px] font-semibold text-muted-foreground uppercase tracking-widest mb-2">{stat.label}</p>
                <div className="flex items-end justify-between">
                  <p className="text-3xl font-black text-foreground tracking-tight leading-none">{stat.value}</p>
        ...
        <div className="grid grid-cols-3 gap-5 flex-1">
          ...
          <div className="col-span-2 space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-[11px] font-semibold text-muted-foreground uppercase tracking-widest">Quick Actions</h2>
            </div>
              ...
              {quickActions.slice(0, 9).map((action) => {
                ...
                return (
                  <button
                    key={action.title}
                    onClick={() => navigate(action.path)}
                    className="group flex items-center gap-3 p-3 bg-card border border-border rounded-xl shadow-sm hover:shadow-md hover:border-primary/30 transition-all text-left"
                  >
                    <div className={`w-8 h-8 rounded-lg ${action.bg} flex items-center justify-center shrink-0 transition-transform group-hover:scale-105`}>
                    ...
                    <ChevronRight className="w-3.5 h-3.5 text-muted-foreground/30 ml-auto shrink-0 group-hover:text-primary transition-colors" />
                  </button>
                );
              })}
            </div>
          ...
          <div className="space-y-4">
            ...
            {attentionStats.length > 0 && (
              <div className="bg-card border border-border rounded-xl shadow-sm overflow-hidden">
                <div className="flex items-center gap-2 px-4 py-3 border-b border-border bg-red-50/50">
                  <AlertTriangle className="w-3.5 h-3.5 text-red-500" />
                  <h3 className="text-[12px] font-semibold text-foreground">Attention Required</h3>
                </div>
                <div className="divide-y divide-border">
                  {attentionStats.map((stat) => (
                    <div
                      ...
                      className="flex items-center justify-between px-4 py-2.5 hover:bg-muted/30 cursor-pointer transition-colors"
                    >
                      <div className="min-w-0">
                        <p className="text-[13px] font-semibold text-foreground">{stat.label}</p>
                        <p className="text-[11px] text-muted-foreground mt-0.5">Needs attention</p>
                      </div>
                      <div className="flex items-center gap-2 shrink-0 ml-3">
                        <span className={`text-[11px] font-black px-2 py-0.5 rounded-full border ${attentionColors[stat.label] || 'text-foreground bg-muted border-border'}`}>
                          {stat.value}
                        </span>
            ...
            <div className="bg-card border border-border rounded-xl shadow-sm overflow-hidden">
              <div className="flex items-center justify-between px-4 py-3 border-b border-border">
                ...
                <button
                  ...
                  className="text-[11px] text-primary font-semibold flex items-center gap-1 hover:gap-1.5 transition-all"
                >
              ...
              <div className="divide-y divide-border">
                {loading ? (
                  <p className="text-center text-xs py-6 text-muted-foreground">Loading tasks...</p>
                ) : upcomingTasks.length === 0 ? (
                  <p className="text-center text-xs py-6 text-muted-foreground">No pending tasks 🎉</p>
                  ...
                  >
                    <div className="flex items-center gap-2 min-w-0">
                      <div className={`w-1.5 h-1.5 rounded-full shrink-0 ${task.status === 'completed' ? 'bg-emerald-500' : 'bg-amber-400'}`} />
                      <div className="min-w-0">
                        <p className="text-[13px] font-medium text-foreground truncate">{toTitleCase(task.title)}</p>
                        <p className="text-[11px] text-muted-foreground">{formatTime(task.time)}</p>
                      </div>
                    </div>
