                ...
                <div className="sm:hidden space-y-3">
                    ...
                    <div className="bg-white rounded-2xl px-4 py-3 border border-slate-100 shadow-sm">
                        <div className="flex items-center justify-between mb-2">
                            <span className="text-[11px] font-black uppercase tracking-wider text-slate-400">Evaluation Progress</span>
                            <span className="text-[11px] font-black text-slate-700">{stats.evaluated}/{stats.total} done</span>
                        </div>
                        ...
                        <div className="flex justify-between mt-1.5">
                            <span className="text-[10px] font-bold text-slate-400">{stats.pending} pending</span>
                            <span className="text-[10px] font-bold text-[#00a67e]">{stats.total > 0 ? Math.round((stats.evaluated / stats.total) * 100) : 0}%</span>
                        </div>
                    </div>

                    {/* Sticky Toolbar */}
                    <div className="sticky top-0 z-20 bg-slate-50/95 backdrop-blur-sm pt-1 pb-2 space-y-2">
                        {/* Search */}
                        <div className="relative">
                            ...
                            <input
                                ...
                                className="w-full bg-white border border-slate-200 rounded-xl pl-9 pr-4 py-2.5 text-[13px] font-medium placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-[#00a67e]/20"
                            />
                        </div>
                        ...
                        <div className="flex items-center gap-2">
                            <div className="flex gap-1.5 flex-1">
                                {(['all', 'pending', 'evaluated'] as const).map(f => (
                                    <button
                                        ...
                                        className={`flex-1 py-1.5 rounded-lg text-[11px] font-black uppercase tracking-wide transition-colors ${
                                            statusFilter === f
                            ...
                            <button
                                ...
                                className="flex items-center gap-1 h-8 px-3 bg-white border border-slate-200 rounded-lg text-[11px] font-black text-slate-600 whitespace-nowrap"
                            >
                                <input
                                    type="checkbox"
                                    readOnly
                                    checked={filteredSubmissions.length > 0 && selectedIds.size === filteredSubmissions.length}
                                    className="w-3.5 h-3.5 accent-[#00a67e]"
                                />
                                All
                            </button>
                        </div>
                    ...
                    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden divide-y divide-slate-50">
                        {filteredSubmissions.length === 0 ? (
                            <div className="p-8 text-center text-sm font-semibold text-slate-400">No students found.</div>
                        ) : (
                            filteredSubmissions.map((sub) => {
                                ...
                                return (
                                    <div
                                        key={sub.id}
                                        className={`flex items-center gap-2.5 px-3 py-2.5 transition-colors ${isSelected ? 'bg-[#00a67e]/5' : isEvaluated ? 'bg-slate-50/50' : 'bg-white'}`}
                                    >
                                        {/* Checkbox */}
                                        <input
                                            type="checkbox"
                                        ...
                                        />
                                        {/* Avatar */}
                                        <div className={`w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-black shrink-0 ${isEvaluated ? 'bg-[#00a67e]/10 text-[#00a67e]' : 'bg-slate-100 text-slate-500'}`}>
                                            {getInitials(sub.studentName)}
                                        </div>
                                        {/* Name + Roll */}
                                        <div className="flex-1 min-w-0">
                                            <p className="text-[13px] font-bold text-slate-800 truncate leading-tight">{sub.studentName}</p>
                                            <p className="text-[10px] font-medium text-slate-400">#{sub.rollNumber}</p>
                                        </div>
                                        {/* Marks input or grade badge */}
                                        {isEvaluated ? (
                                            <div className="flex items-center gap-1.5 shrink-0">
                                                <span className="text-[12px] font-black text-[#00a67e]">{sub.marksObtained}/{work.maxMarks}</span>
                                                <button
                                            ...
                                            <div className="flex items-center gap-1.5 shrink-0">
                                                <div className="relative w-14">
                                                    <input
                                                        ...
                                                        placeholder="—"
                                                        className="w-full h-8 text-center text-[13px] font-black bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#00a67e]/30"
                                                    />
                                                </div>
                                                <span className="text-[10px] text-slate-400 font-bold">/{work.maxMarks}</span>
                                                <button
                                                    onClick={() => handleInlineSave(sub, mobilePendingList)}
                                                    disabled={isSaving || !inlineMarks[sub.id]}
                                                    className="w-8 h-8 flex items-center justify-center rounded-lg bg-[#00a67e] text-white disabled:opacity-40 transition-opacity"
                                                >
                                                    {isSaving ? (
